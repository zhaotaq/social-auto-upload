...
async def run_workflow(config_path: str):
    """Runs the multi-account and multi-video-type workflow."""
    print(f"Loading workflow config from {config_path}")
        ...
        for video_type in video_types:
            ...
            for index, video_file in enumerate(video_files):
                ...
                for platform in platforms:
                    ...
                    # Call the appropriate uploader based on platform
                    try:
                        if platform == SOCIAL_MEDIA_DOUYIN:
                            ...
                            await app.main()
                            print(f"      Successfully initiated upload for {video_file.name} to {platform} for account {account_name}")
                            ...
                            app.upload() # Bilibili example used synchronous upload
                            print(f"      Successfully initiated upload for {video_file.name} to {platform} for account {account_name}")
                            # Add a small delay after Bilibili upload as in example
                            time.sleep(10) # Reduced delay from 30s in example
                        ...
                        print(f"      Error processing {video_file.name} for {platform} (Account: {account_name}): {e}")
                         ...
                         print(f"      An unexpected error occurred during upload for {video_file.name} to {platform} (Account: {account_name}): {e}")
                         # Optionally, add more specific error handling based on uploader exceptions

                    # Add a small delay between uploads to different platforms for the same video
...
# Modified run_workflow to accept config dictionary or path
